...
import {fetchBases, fetchBrands, fetchTechs, fetchTypes, updateTech} from "../../http/techAPI";
import {observer} from "mobx-react-lite";


const UpdateTech = observer(({ show, onHide }) => {
    ...
    const update = ()=>{
        ...
        //formData.append("techId",tech.selectedTeches.id);
            ...
            <Modal.Body>
                <Form>
                    <Dropdown className="mt-2 mb-2">
                        <Dropdown.Toggle>{tech.selectedTeches.name|| "Выберите спецтехнику"}</Dropdown.Toggle>
                        <Dropdown.Menu>
                            {tech.teches.map((t) => (
                                <Dropdown.Item onClick={()=>tech.setSelectedTeches(t)} key={t.id}>{t.name}</Dropdown.Item>
                            ))}
                        </Dropdown.Menu>
                    </Dropdown>
                    <Dropdown className="mt-2 mb-2">
